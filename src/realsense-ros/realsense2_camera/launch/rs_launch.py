...
def generate_launch_description():
    ...
    if (os.getenv('ROS_DISTRO') == "dashing") or (os.getenv('ROS_DISTRO') == "eloquent"):
        return LaunchDescription(declare_configurable_parameters(configurable_parameters) + [
            ...
            launch_ros.actions.Node(package='tf2_ros',
                    node_executable='static_transform_publisher',
                    node_name='imu_optical_to_ros_frame',
                    arguments=['0', '0', '0', '0.5', '-0.5', '0.5', '0.5', 'camera_gyro_optical_frame', 'camera_imu_ros_frame'],
                    ),
            launch_ros.actions.Node(
                package='realsense2_camera',
                node_name='imu_optical_to_ros',
                node_executable='imu_optical_to_ros',
                output='screen',
                )
            ])
    else:
